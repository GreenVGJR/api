...
const startupDataPromise = Promise.all([
	...
	fs
		.readFile(path.join(__dirname, "node_modules/hono/package.json"), "utf-8")
		.catch(() => ""),
...
const API_ROUTES = {
	...
	discord_tools: {
		...
		server: [
			...
			[
				...
				"json",
				"string",
				"enum_multi:PROFANITY,SEXUAL_CONTENT,SLURS",
				"string",
				"number",
				"boolean",
				"json",
				"enum_multi:BLOCK_MESSAGE,SEND_ALERT_MESSAGE,TIMEOUT,BLOCK_MEMBER_INTERACTION",
				"number",
				"number",
				"string",
				"string",
		...
		member: [
			...
			[
				...
				"number",
				"number",
				"enum_multi:user,bot,all,oldest,newest,no_role,has_role,banned",
				"string",
			],
			[
				...
				"number",
				"number",
				"enum_multi:user,bot,all,oldest,newest,oldest_position,newest_position",
				"string",
			],
		],
		channel: [
			[
				...
				"number",
				"number",
				"enum_multi:text,voice,category,announcement,announcement_thread,public_thread,private_thread,stage,directory,forum,media,threads,all",
			],
			[
		...
		invite: [
			...
			[
				...
				"number",
				"number",
				"enum_multi:user,bot,all,oldest,newest,temporary,permanent,has_expire",
				"number",
			],
		],
		sticker: [
	...
	music: [
		...
		[
			...
			"string",
			"string",
			"enum:youtube,youtubemusic,soundcloud,spotify,applemusic,deezer,tidal",
			"number",
			"number",
			"number",
			"boolean",
		...
		[
			...
			"string",
			"number",
			"enum:off,track,queue,autoplay,toggle,0,1,2,3",
		],
		...
		[
			...
			"string",
			"number",
			"enum:nightcore,vaporwave,speed,slow,chipmunk,deep,bassboost,bassboostlow,bassboosthigh,soft,trebleboost,rock,pop,electronic,classical,vocal,vocalonly,fullsound,gaming,8d,karaoke,tremolo,pulse,vibrato,wobble,lowpass,muffled,rotation,spin,distortion,channelmix,mono,wide,surround,left,right,reset",
		],
		[
			"/music/voiceStatus?token=&guildId=&type=&status=&content=",
			"string",
			"number",
			"enum:trackStart,queueEnd",
			"boolean",
			"string",
		],
	],
...
app.use("*", async (c: Context, next: Next) => {
	...
	if (getCookie(c, "cf_clearance")) {
		...
		c.header(
			"Set-Cookie",
			`cf_clearance=; Max-Age=0; Expires=${expiry}; Domain=${domain}; Path=/; Secure; HttpOnly; SameSite=None; Partitioned;`,
			{ append: true },
		);
		c.header(
			"Set-Cookie",
			`cf_clearance=; Max-Age=0; Expires=${expiry}; Path=/; Secure; HttpOnly; SameSite=None; Partitioned;`,
			{ append: true },
		);
	}
...
function isBackChallengePath(pathname: string): boolean {
	...
	return BACK_CHALLENGE_PREFIXES.some(
...
app.get("/", (c: Context) =>
	stream(c, async (l) => {
		...
		const os_uptime = [os_h, os_m, os_s]
			.map((v) => v.toString().padStart(2, "0"))
			.join(":");
		...
		await l.write(
			renderJson ? JSON.stringify(listapi) : JSON.stringify(listapi, null, 2),
		);
	}),
);

const routeBase = BUILD_ID ? `/${BUILD_ID}` : "";
const apiPrefixesRoute = [
	"/search",
	"/lyrics",
	"/tools",
	"/info",
