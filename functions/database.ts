...
function encrypt(text: string): string {
    try {
        const key = crypto.createHash('sha256').update(DB_KEY as string).digest();
...
export default {
    ...
    getAll(query: string = "", hash?: string) {
        ...
        if (!query) {
            ...
            results = stmt.all(hash) as { name: string, value: string, hash: string }[];
        } else {
            const stmt = db.prepare("SELECT name, value, hash FROM apidataobjects WHERE name LIKE ? AND hash = ?");
            results = stmt.all(`%${query}%`, hash) as { name: string, value: string, hash: string }[];
        }
