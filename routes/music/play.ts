    ...
    switch (platform) {
      ...
      case "spotify": {
        ...
        return {
          ...
          title: track.name,
          author: track.artists?.items
            ?.map((a: any) => a.profile?.name)
            .join(", "),
          thumbnail: track.albumOfTrack?.coverArt?.sources?.sort(
...
app.get("/play", async (c) => {
  return await createMusicStream(c, async (log, s) => {
    ...
    const platform = (c.req.query("platform") || "youtubemusic")
      .toLowerCase()
      .replace(/\s+/g, "");
    ...
    if (!token || !query || (!voiceId && (!reqGuildId || !authorId))) {
      await s.write(
        `],"data":${JSON.stringify({ status: false, message: "Missing required params: token, q, and either voiceId OR (guildId and authorId)", type: { primary: "error", alt: "invalid_query" } })}}`,
      );
      return;
    }
    ...
    const isUrl =
      queryStr.startsWith("http://") || queryStr.startsWith("https://");
    ...
    if (!isUrl && !supportedPlatforms.includes(platform)) {
      ...
      await s.write(
        `],"data":${JSON.stringify({ status: false, message: `Search engine "${platform}" is not supported.`, list: supportedPlatforms.join(", "), type: { primary: "error", alt: "invalid_query" } })}}`,
      );
      return;
    }
    ...
    const effectivePlatform = isUrl
      ? getPlatformFromUrl(queryStr) || platform
      : platform;
    ...
    const pCustom = customSearchWithTimeout(
      ...
      log,
      isUrl
        ? "[Attempt 1] URL metadata lookup"
        : `[Attempt 1] Custom ${platform} search`,
    );
    ...
    const applyOverlay = (
      result: any,
      customResult: CustomSearchResult | null,
    ) => {
      if (result?.tracks?.[0] && customResult) {
        ...
        t.info = {
          ...
          ...(customResult.thumbnail
            ? { artworkUrl: customResult.thumbnail }
            : {}),
          actualSourceName: t.info.sourceName,
          ...(customResult.sourceName
            ? { sourceName: customResult.sourceName }
            : {}),
    ...
    const pSearch = (async () => {
      ...
      const doSearch = async (q: string, src: string) => {
        ...
        if (!res?.tracks?.length)
          throw new Error(
            `No results for "${normalizedQ}" due unavailable or geo-restriction`,
          );
        return res;
      };

      const startLavalinkPlatformSearch = (
      ...
      const useLavalinkPlatformSearch = async (
        ...
        await log(
          `[Attempt ${attempt}] Lavalink ${label} search failed (${error?.message || "No results"})`,
        );
        return null;
      };

      const pPrimaryLavalink =
        !isUrl && allowFallback
          ? startLavalinkPlatformSearch(platform, queryStr)
          : null;
      const customResult = await pCustom;

      let result: any = null;
      const loadSoundCloudManual = async (
        sourceUrl: string,
        attempt: string | number,
        overlaySource: CustomSearchResult | null,
      ): Promise<boolean> => {
        ...
        for (const stream of streams) {
          const protocol =
            stream.protocol === "progressive"
              ? "progressive/legacy"
              : stream.protocol;
          const mime = stream.mimeType ? ` (${stream.mimeType})` : "";
          await log(
            `[Attempt ${attempt}] Manual ${protocol}${mime} stream resolved — loading via Lavalink`,
          );
          try {
            result = await doSearch(stream.url, "url");
            applyOverlay(result, overlaySource);
            await log(
              `[Attempt ${attempt}] "SoundCloud Manual ${protocol}" succeeded`,
            );
            return true;
          } catch (err: any) {
            await log(
              `[Attempt ${attempt}] "SoundCloud Manual ${protocol}" failed: ${err?.message || err}`,
            );
          }
        }
      ...
      if (isUrl) {
        ...
        try {
          if (isYtMix) {
            await log(
              `[Attempt 1] YouTube Mix (list=${ytListParam}) detected — fetching track list from watch page...`,
            );
            const splitqueryStr = new URL(queryStr);
            const mixTrackUrls = await fetchYouTubeMixTracks(
              `https://youtube.com/watch?v=&list=${splitqueryStr.searchParams.get("list")}`,
            );
            if (mixTrackUrls?.list?.length) {
              await log(
                `[Attempt 1] Fetched ${mixTrackUrls.list.length} tracks from mix — loading individually...`,
              );
              ...
              if (tracks.length > 0) {
                ...
                await log(
                  `[Attempt 1] Mix loaded: ${tracks.length}/${mixTrackUrls.list.length} tracks`,
                );
              } else {
            ...
            await log(
              `[Attempt 1] YouTube playlist detected. Trying playlist first: "${ytPlaylistUrl}"`,
            );
          ...
          if (effectivePlatform === "soundcloud") {
            await log(
              `[Attempt 1] SoundCloud direct load failed — attempting manual stream resolution`,
            );
          ...
          if (!result && ytVideoUrl) {
            await log(
              `[Attempt 1] YouTube playlist load failed (${e?.message}) — falling back to video-only URL`,
            );
            try {
              ...
              await log(
                `[Attempt 2] Video-only fallback also failed (${e2?.message}) — giving up`,
              );
            }
          }
        }
      ...
      if (isUrl && !result && allowFallback && !ytPlaylistUrl && !isYtMix) {
        const metaQuery = customResult?.title
          ? customResult.author
            ? `${customResult.title} ${customResult.author}`
            : customResult.title
          : null;

        if (!metaQuery || !result) {
          await log(
            `[Attempt 1] Could not extract track title from URL — skipping fallback search`,
          );
          throw new Error(
            `Direct load failed and no metadata found for "${queryStr}"`,
          );
        }

        await log(
          `[Attempt 1] Extracted metadata: "${metaQuery}" — starting fallback search`,
        );
        ...
        for (const attempt of urlFallbacks) {
          const pFallbackLavalink = startLavalinkPlatformSearch(
            attempt.searchPlatform,
            attempt.query,
          );
          await log(
            `[Attempt ${currentAttempt}] Custom ${attempt.label} search: "${attempt.query}"`,
          );
          try {
            ...
            if (!fallbackResult?.url) {
              await log(
                `[Attempt ${currentAttempt}] "${attempt.label}" returned no results`,
              );
            } else {
              await log(
                `[Attempt ${currentAttempt}] Got URL: "${fallbackResult.url}" — loading via Lavalink`,
              );
              try {
                result = await doSearch(fallbackResult.url, "url");
                applyOverlay(result, customResult ?? fallbackResult);
                await log(
                  `[Attempt ${currentAttempt}] "${attempt.label}" succeeded`,
                );
                break;
              } catch (err: any) {
                if (
                  attempt.searchPlatform === "soundcloud" &&
                  fallbackResult.url
                ) {
                  await log(
                    `[Attempt ${currentAttempt}] SoundCloud direct load failed — attempting manual stream resolution`,
                  );
                  if (
                    await loadSoundCloudManual(
                      fallbackResult.url,
                      currentAttempt,
                ...
                await log(
                  `[Attempt ${currentAttempt}] "${attempt.label}" failed: ${err?.message}`,
                );
              }
            }
          } catch (err: any) {
          ...
          currentAttempt++;
          result = await useLavalinkPlatformSearch(
        ...
        if (customResult) {
          await log(
            `[Attempt 1] Got URL: "${customResult.url}" — loading via Lavalink`,
          );
          try {
            ...
            if (platform === "soundcloud" && customResult?.url) {
              await log(
                `[Attempt 1] SoundCloud direct load failed (${e?.message || e}) — attempting manual stream resolution`,
              );
              ...
              await log(
                `[Attempt 1] ${platform} direct URL load failed (${e?.message || e})${allowFallback ? " — falling back to Lavalink search" : ""}`,
              );
            }
          }
        } else {
          await log(
            `[Attempt 1] No URL returned${allowFallback ? " — falling back to Lavalink search" : " — fallback disabled"}`,
          );
        }
    ...
    const [searchResult, setup, _] = await Promise.allSettled([
      pSearch,
      pGP,
      pConnect,
    ]);
    ...
    if (searchResult.status === "rejected") {
      await s.write(
        `],"data":${JSON.stringify({ status: false, message: searchResult.reason?.message || "Search failed", type: { primary: "error", alt: "critical" } })}}`,
      );
      return;
    }
    ...
    if (setup.status === "rejected" || !setup.value) {
      await log(
        setup.status === "rejected"
          ? `Voice setup failed: ${setup.reason?.message}`
          : "No voice channel found",
      );
      await s.write(
        `],"data":${JSON.stringify({ status: false, message: setup.status === "rejected" ? setup.reason?.message : "Cant find a voice channel", type: { primary: "error", alt: "unknown_voice" } })}}`,
      );
      return;
    }
    ...
    if (isPlaylist) {
      const playlistName =
        searchResult.value.playlist?.name || "Unknown Playlist";
      const playlistUrl =
        searchResult.value.playlist?.uri ||
        searchResult.value.playlist?.url ||
        "";
      ...
      const filteredTracks = tracks.filter(
        (t: any) => !t.info.isStream || isSoundCloudTrack(t),
      );
      if (filteredTracks.length === 0) {
        await log("No non-live tracks found in playlist");
        await s.write(
          `],"data":${JSON.stringify({ status: false, message: "Playlist contains only live tracks, which are not allowed", type: { primary: "error", alt: "invalid_query" } })}}`,
        );
        return;
      }
      ...
      if (guildPlayer.get("autoplay")) {
        const autoplayIndex = guildPlayer.queue.tracks.findIndex(
      ...
      if (track.info.isStream && !isSoundCloudTrack(track)) {
        ...
        await s.write(
          `],"data":${JSON.stringify({ status: false, message: "Live tracks (streams) are not allowed", type: { primary: "error", alt: "invalid_query" } })}}`,
        );
        return;
      }
      responseTrack = track;
      ...
      try {
        await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Play request timed out after 30s")),
              30_000,
            ),
          ),
        ]);
        ...
        await s.write(
          `],"data":${JSON.stringify({ status: false, message: err?.message || "Failed to play track", type: { primary: "error", alt: "critical" } })}}`,
        );
        return;
      }
    }
    ...
    const queueTracks = guildPlayer.queue.tracks.slice(0, 3).map((t) => {
      ...
      return track.info?.identifier === responseTrack.info?.identifier
        ? formattedTrack
        : formatTrack(track, client, guildPlayer);
    });
    const totalQueueDuration = guildPlayer.queue.tracks.reduce(
