...
app.get('/discord/infoMessage', async (c) => {
    ...
    const messageId = (queryMessageId && Number.isInteger(parseInt(queryMessageId))) ? queryMessageId : null;
    ...
    if (!messageId) return c.json({ error: "Missing valid parameter: messageId" }, 202);

    c.header('X-Route', 'discord.com');
    return await dispatch(c, () => DiscordInfoMessage(token!, channelId!, messageId!));
});
