...
import { DiscordModifyRole } from "../../functions/request.js";
import { dispatch, processImage } from "../../functions/httpRequest.js";

app.get("/discord/modifyRole", async (c) => {
	...
	const position = getQuery("position");
	...
	if (position !== undefined) {
		const posVal = parseInt(position);
		if (!isNaN(posVal) && posVal >= 0) {
			payload.position = posVal;
		} else {
			payloadError.push("[position] Invalid position value");
		}
	}
	...
	return await dispatch(c, () => DiscordModifyRole(token!, roleId!, guildId!, payload, reason || undefined));
});
